...
# 同时保存为静态图片格式
output_formats = {
    'png': "btc_cycles_comparison.png",
    'jpg': "btc_cycles_comparison.jpg",
    'svg': "btc_cycles_comparison.svg",
    'pdf': "btc_cycles_comparison.pdf"
}
...
for fmt, filename in output_formats.items():
    try:
        ...
        print(f"{fmt.upper()} 图片已保存为: {filename}")
    except Exception as e:
        print(f"保存为 {fmt} 格式时出错: {str(e)}")

print(f"请在Windows文件管理器中打开这些文件查看。")
